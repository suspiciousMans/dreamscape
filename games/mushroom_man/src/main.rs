...
fn entrance_level() -> Level {
    ...
    // A friendly guide near the spawn point — the vertical slice's one
    // dialogue-bearing character, hinting at the Bounce gate ahead rather
    // than leaving the player to guess what the mushroom does.
    let cave_sprite = CharacterInstance {
        name: "Cave Sprite".to_string(),
        ...
        // A small branching demo: the greeting offers a choice, both
        // branches loop back to the greeting afterward (linear nodes wrap
        // via `Dialogue::advance`, same as the old flat-cycling behavior).
        dialogue_nodes: vec![
            DialogueNode {
                text: "Oh! A visitor. Need help with something?".to_string(),
                choices: vec![
                    ("Tell me about the mushroom".to_string(), 1),
                    ("No thanks, I'm good".to_string(), 2),
                ],
            },
            DialogueNode {
                text: "Eat the red mushroom to bounce sky-high — you'll need it to reach the tunnels.".to_string(),
                choices: Vec::new(),
            },
            DialogueNode { text: "Good luck down there. It gets dark.".to_string(), choices: Vec::new() },
        ],
    };
...
fn tunnels_level() -> Level {
    ...
    // A shy critter wandering the open floor before the crawl gate — no
    // health/damage, it just flees if the player gets close, giving the
    // corridor a bit of ambient life without any combat stakes.
    let tunnel_crawler = CharacterInstance {
        name: "Tunnel Crawler".to_string(),
    ...
    // A demo of the spawner/wave system — a nest that periodically adds
    // more crawlers (capped at 2 alive at once) so the corridor doesn't
    // stay quiet forever if the player lingers or the original crawler
    // wanders off-screen.
    let crawler_nest = SpawnerInstance {
        name: "Crawler Nest".to_string(),
    ...
    Level {
        ...
        particle_emitters: Vec::new(),
        rig_instances: Vec::new(),
        characters: vec![tunnel_crawler],
        spawners: vec![crawler_nest],
        physics: PhysicsParams::default(),
        music_path: Some(PathBuf::from("music/demo_ambient.wav")),
    }
}
...
fn grotto_level() -> Level {
    ...
    // Guards the approach to the Heart of the Grove — reaching the goal
    // means fighting past it or slipping by while it's distracted, giving
    // the vertical slice's ending an actual stake beyond just walking up
    // to a trigger.
    let spore_guardian = CharacterInstance {
        name: "Spore Guardian".to_string(),
...
struct Sandbox {
    ...
    /// doesn't bring back things the player already removed (e.g. an
    /// eaten mushroom). Cleared by `apply_level`.
...
impl Sandbox {
    ...
    fn apply_level(&mut self, gl: &glow::Context, level: &Level) -> anyhow::Result<()> {
        ...
        log::info!(
            "loaded level '{}' ({} objects, {} lights, {} particle emitters, {} rig instances, {} characters, {} spawners)",
            level.name,
            level.objects.len(),
            level.lights.len(),
    ...
    fn save_checkpoint(&mut self) {
        ...
        if let Some(kind) = self.active_mushroom {
            extra.insert("active_mushroom".to_string(), kind as u8 as f32);
        }
        let data = SaveData {
    ...
    fn load_checkpoint(&mut self, ctx: &mut Context) {
        ...
        // Re-despawn whatever was already removed at save time (e.g. an
        // eaten mushroom pickup) — `apply_level` just freshly respawned it.
        ...
        if let Some(&kind_f) = data.extra.get("active_mushroom") {
            if let Some(kind) = MushroomKind::all().into_iter().find(|k| *k as u8 as f32 == kind_f) {
    ...
    fn on_trigger_entered(&mut self, gl: &glow::Context, trigger: Entity) -> bool {
        ...
        if let Some(kind) = mushroom_kind {
            self.eat_mushroom(kind);
            if let Ok(meta) = self.world.get::<&LevelObjectMeta>(trigger) {
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(script_path) = smoke_test_path_from_args() {
        ...
        let code = App::run_scripted("Jame Engine mushroom_man", 1280, 720, Sandbox::new(), script)?;
